(() => {
  ...
  if (urlToolIds && isComparePage) {
    const fromUrl = urlToolIds
      ...
      .map(item => {
        try {
          return decodeURIComponent(item).trim().toLowerCase();
        } catch (_) {
          return '';
        }
      })
  ...
  const setupPickerEnhancements = () => {
    ...
    if (window.__toolVaultCompareSearchInput) {
      const search =
    ...
    window.__toolVaultCompareSearchInput = true;
  ...
  const wrapRenderer = () => {
    if (
      !isComparePage ||
      typeof window.renderComparePage !== 'function' ||
  ...
  /*
   * IMPORTANT:
   * Capture comparison interactions before any surrounding
   * or global navigation handler can process the same click.
   *
   * This specifically prevents a Compare button from falling
   * through to unrelated navigation such as Contact.
   */
  document.addEventListener(
    ...
    event => {
       ...
       * Explicitly keep it on compare.html and prevent
       * unrelated delegated handlers from changing destination.
       */
      const compareLink =
