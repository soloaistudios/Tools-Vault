(() => {
  ...
  if (
    urlToolIds &&
    location.pathname.toLowerCase().endsWith('/compare.html')
  ) {
    const fromUrl = urlToolIds
      .split(',')
      .map(item => decodeURIComponent(item).trim().toLowerCase())
      .filter(Boolean);
      ...
      <div class="compare-bar-inner">
        ...
        <div class="compare-bar-actions">
          ...
          <a
            ...
            href="compare.html?tools=${encodeURIComponent(
              selection.join(',')
            )}#compare-results"
          >
            ${
  ...
  const syncButtons = () => {
    ...
    document
      ...
      .forEach(input => {
        input.checked = selection.includes(
          input.value
        );
  ...
  /*
   * Capture comparison interactions before any
   * other document-level click handler.
   *
   * This prevents comparison controls from
   * falling through to unrelated navigation
   * such as Contact.
   */
