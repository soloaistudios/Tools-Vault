(() => {
  ...
  const syncButtons = () => {
    ...
    document
      .querySelectorAll('[data-compare-pick]')
      .forEach(input => {
        input.checked =
          selection.includes(
            input.value
  ...
  const setupPickerEnhancements = () => {
    ...
    if (
      window.__toolVaultCompareSearchBound
    ) {
      filterPickerCards();
      return;
    }
    ...
    if (
      window.__toolVaultCompareSearchInput
    ) {
      window.__toolVaultCompareSearchInput.value =
        pickerSearch;
  ...
  const enhanceRenderedResults = () => {
    ...
    enhanceRenderedResults.__ready =
      true;
  ...
  const wrapRenderer = () => {
    if (
      ...
      typeof window.renderComparePage !==
        'function' ||
      window.__toolVaultCompareWrapped
    ) {
      return;
    }
    ...
    window.renderComparePage =
      function wrappedComparePage(
        nextSelection
      ) {
        original(nextSelection);
  ...
  /*
   * Capture-phase handling prevents surrounding
   * document handlers from treating comparison
   * controls as unrelated navigation.
   */
  document.addEventListener(
    ...
    event => {
      ...
      if (clear) {
        ...
        /*
         * Clear the comparison URL as well
         * so a later refresh doesn't restore
       ...
       * localStorage and return to the
       * catalog.
       */
      const addAnother =
        target.closest(
