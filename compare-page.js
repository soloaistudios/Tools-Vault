(() => {
  ...
  const updateComparisonCue = selected => {
    const existing = document.getElementById('compareReadyCue');
    if (existing) existing.remove();
    if (selected.length < 2) return;

    const cue = document.createElement('div');
    cue.id = 'compareReadyCue';
    cue.className = 'compare-ready-cue';
    cue.innerHTML = `
      <div class="compare-ready-copy">
        <span class="compare-ready-icon" aria-hidden="true">✓</span>
        <div>
          <strong>${selected.length} tools selected — your comparison is ready</strong>
          <span>Your side-by-side table is just below. Open it when you are ready.</span>
        </div>
      </div>
      <button type="button" class="btn btn-primary btn-small" data-compare-jump>See Comparison <span>↓</span></button>`;

    const panel = document.querySelector('.compare-picker-panel');
    panel?.insertAdjacentElement('afterend', cue);
  };

  const jumpToResults = () => {
    results.scrollIntoView({ behavior: 'smooth', block: 'start' });
    window.history.replaceState(null, '', `${location.pathname}${location.search}#compare-results`);
  };
  ...
  window.renderComparePage = selection => {
    ...
    document.querySelectorAll('[data-compare-submit]').forEach(button => {
      button.innerHTML = selected.length >= 2
        ? 'See Comparison <span>↓</span>'
        : 'Run Comparison <span>→</span>';
    });

    if (selected.length >= 2 && location.hash === '#compare-results') {
      requestAnimationFrame(() => requestAnimationFrame(jumpToResults));
    }
  };

  document.addEventListener('click', event => {
    const jump = event.target.closest?.('[data-compare-jump]');
    if (jump) {
