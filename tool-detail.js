(() => {
  ...
  const data = window.toolVault?.TOOL_DATA || window.TOOL_DATA;
  ...
  const related = data
    ...
    .map(item => `
      <a class="related-tool-card" href="tool.html?id=${encodeURIComponent(item.id)}">
        <div class="tool-logo ${escapeHTML(item.id)}-logo">${escapeHTML(item.name.charAt(0))}</div>
        <div>
    ...
    <div class="tool-detail-page">
      <section class="tool-detail-hero detail-hero-card">
        <div class="detail-hero-brand">
          <div class="tool-logo detail-logo ${escapeHTML(tool.id)}-logo">${escapeHTML(tool.name.charAt(0))}</div>
